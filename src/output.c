...
void editorDrawRows(struct abuf *ab) {
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    // This runs if the current row is is greater than the amount of rows in the
    // file.
    if (filerow >= E.numrows) {
      ...
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        // snprintf returns the size of the given array (welcome) plus the
        // string literal and the arguments passed to it, in this case
        // EVIM_VERSION.
      ...
      // If the file has one or more lines then it fills the rest of the lines
      // with ~
      else {
        abAppend(ab, "~", 1);
      }
    ...
    // Aka it just prints the row.
      ...
      // So say a line is 10 chars long and the current coloumn offset is 6 then
      // it would start printing the line from the 6th char. This enables
...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  // Sets the color to red for insert mode
  // Because it is the danger zone
  else if (E.currentMode == INSERT_MODE) {
    abAppend(ab, "\x1b[97;41m", 8);
  }
  // Sets background color to white in command mode
  ...
  int len = snprintf(status, sizeof(status), "%.20s - %d lines %s - %s mode",
                     E.filename ? E.filename : "[No Name]", E.numrows,
                     E.dirty ? "(modified)" : "", E.modes[E.currentMode].name);

  // Calculates the length of the posistion string.
  int poslen = snprintf(posstatus, sizeof(posstatus), "%s -- %d:%d",
                        E.mBuffer.buffer, E.cy + 1, E.cx + 1);
  ...
  while (len < E.screencols) {
    // Checks if the length of the posstatus string literal is equal to the
    // amount of space left. If it is then it adds the posistion status to the
    // buffer.
...
void editorDrawMessageBar(struct abuf *ab) {
  ...
  // Checks if the msglen is non-zero and if the amount of time that has passed
  // since the status msg has been displayed is less than 5 sec. If true then
  // add statusmsg to buffer.
...
void editorRefreshScreen() {
  ...
  // Sets the current cursor position to the one specified in the editorconfig
  // struct
  char buff[32];
...
// The "..." make this a varidic function and means it can take a variable
// amount of arguments. fmt is a formated string. Could be something like
// "Hello, %s!", the s would be passed as a extra argument.
